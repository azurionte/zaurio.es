...
export const BANK_SYNC_SERVICE_VERSION='dz3-bank-sync-1';
...
export async function syncBankProvider({repository,provider,connection,planId,expectedEvents=[]}){
...
export async function acceptReconciliation({repository,planId,proposal,status='user_confirmed'}){
  ...
  const{data:match,error}=await repository.client.from('dz3_reconciliation_matches').insert({plan_id:planId,bank_transaction_id:bankTransaction.id,financial_event_id:null,match_type:'event',confidence:proposal.confidence,status,metadata:{signals:proposal.signals||{}}}).select().single();
  if(error)throw error;
