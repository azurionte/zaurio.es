...
export function generateDebtLedger({ debts = [], debtSchedules = [], recurrenceRules = [], debtAdjustments = [], from, to }) {
  const schedules=new Map(debtSchedules.map(row=>[row.debtId,row]));
  const recurrences=mapById(recurrenceRules);
  return debts.flatMap(debt=>{
    const schedule=schedules.get(debt.id); if(!schedule)return[];
    const recurrence=schedule.recurrenceId?recurrences.get(schedule.recurrenceId):null;if(!recurrence)return[];
    return generateDebtEvents({debt,schedule,recurrenceRule:{frequency:recurrence.frequency,intervalValue:recurrence.intervalValue,anchorDate:recurrence.anchorDate,endDate:recurrence.endDate,calendarRule:recurrence.calendarRule,dueDay:recurrence.dueDay,leadDays:recurrence.leadDays},adjustments:debtAdjustments,from,to});
  });
}
export function buildFinancialState({range,plan,salaryEvent=null,nextSalaryEvent=null,incomeRules=[],expenseRules=[],savingsGoals=[],recurrenceRules=[],debts=[],debtSchedules=[],debtAdjustments=[],eventOverrides=[],ruleVersions=[],confirmedEvents=[],transferEvents=[],accounts=[],buckets=[],transfers=[],observations=[],asOf=range.to}){
  const debtEvents=generateDebtLedger({debts,debtSchedules,recurrenceRules,debtAdjustments,from:range.from,to:range.to});
  ...
  const ledger=[...expected,...confirmedEvents];
  const summary=summarizeLedger(ledger,range);
  const explanation=explainAggregate(ledger,range);
  const accounting=resolveAccountState({accounts,buckets,events:ledger,transfers,observations,from:range.from,asOf});
  assertAccountingInvariants(accounting);
  const analytics=analyzePeriod({events:ledger,from:range.from,to:range.to});
  const period=plan?resolvePeriod({mode:plan.periodMode,labelMonth:plan.labelMonth||range.from.slice(0,7),salaryEvent,nextSalaryEvent,fundingStrategy:plan.salaryFundingStrategy}):null;
